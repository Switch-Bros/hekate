...
void print_fuseinfo()
{
	...
	gfx_printf("SD-RAM ID:    %d\n", fuse_read_dramid(true));
	gfx_printf("Verbrauchte fuses: %d / 64\n", bit_count(fuse_read_odm(7)));
	gfx_printf("Sicherheitsschluessel:  %08X%08X%08X%08X\n\n\n",
...
void print_mmc_info()
{
	...
	{
		EPRINTF("eMMC konnte nicht inizialisiert werden.");
		goto out;
	}
	else
	{
		...
		if (emmc_storage.csd.structure == 0)
			EPRINTF("Unbekannte CSD Struktur.");
		else
		{
			gfx_printf("%kErweitertes CSD V1.%d:%k\n",
			...
			gfx_printf(
				...
				emmc_storage.csd.capacity == (4096 * 512) ? "Hoch" : "Niedrig", speed & 0xFFFF, (speed >> 16) & 0xFFFF,
			...
			gfx_printf("%keMMC Partitionen:%k\n", TXT_CLR_CYAN_L, TXT_CLR_DEFAULT);
			gfx_printf(" 1: %kBOOT0      %k\n    Groesse: %5d KiB (LBA Sektoren: 0x%07X)\n", TXT_CLR_GREENISH, TXT_CLR_DEFAULT,
...
void print_sdcard_info()
{
	...
	{
		gfx_printf("%kKarten IDentifikation:%k\n", TXT_CLR_CYAN_L, TXT_CLR_DEFAULT);
		gfx_printf(
			" Hersteller ID:  %02x\n"
			" OEM ID:         %c%c\n"
			" Modell:         %c%c%c%c%c\n"
		...
		gfx_printf("%kSD-Karte Spezifische Daten V%d.0:%k\n", TXT_CLR_CYAN_L, sd_storage.csd.structure + 1, TXT_CLR_DEFAULT);
		gfx_printf(
			" Cmd Klassen:     %02X\n"
			" Kapazitaet:      %d MiB\n"
			" Busbreite:       %d\n"
			" Aktuelle Rate:   %d MB/s (%d MHz)\n"
			" Speed Class:     %d\n"
			" UHS Grade:       U%d\n"
			" Video Class:     V%d\n"
			" App perf class:  A%d\n"
		...
		if (!res)
		{
			gfx_puts("Abrufen von FAT-Volumeninformationen...\n\n");
			f_getfree("", &sd_fs.free_clst, NULL);
			gfx_printf("%kGefunden %s Volume:%k\n Frei:    %d MiB\n Cluster: %d KiB\n",
	...
	{
		EPRINTF("SD-Karte konnt nicht inizialisiert werden.");
		if (!sdmmc_get_sd_inserted())
			EPRINTF("Stelle sicher das sie eingesteckt ist.");
		else
			EPRINTF("SD-Kartenleser sitzt nicht korrekt!");
		sd_end();
	}
...
void print_fuel_gauge_info()
{
	...
	gfx_printf("Kapazitaet jetzt:           %3d%\n", value >> 8);

	max17050_get_property(MAX17050_RepCap, &value);
	gfx_printf("Kapazitaet jetzt:           %4d mAh\n", value);

	max17050_get_property(MAX17050_FullCAP, &value);
	gfx_printf("Kapazitaet voll:          %4d mAh\n", value);

	max17050_get_property(MAX17050_DesignCap, &value);
	gfx_printf("Kapazitaet (design):      %4d mAh\n", value);

	max17050_get_property(MAX17050_Current, &value);
	gfx_printf("Momentan jetzt:            %d mA\n", value / 1000);

	max17050_get_property(MAX17050_AvgCurrent, &value);
	gfx_printf("Momentan Durchschnitt:        %d mA\n", value / 1000);

	max17050_get_property(MAX17050_VCELL, &value);
	gfx_printf("Spannung jetzt:            %4d mV\n", value);

	max17050_get_property(MAX17050_OCVInternal, &value);
	gfx_printf("Leerlaufspannung:   %4d mV\n", value);

	max17050_get_property(MAX17050_MinVolt, &value);
	gfx_printf("Min. erreichte Spannung:    %4d mV\n", value);

	max17050_get_property(MAX17050_MaxVolt, &value);
	gfx_printf("Max. erreichte Spannung:    %4d mV\n", value);

	max17050_get_property(MAX17050_V_empty, &value);
	gfx_printf("Leerspannung (design): %4d mV\n", value);

	max17050_get_property(MAX17050_TEMP, &value);
	gfx_printf("Akku Temperatur:    %d.%d oC\n", value / 10,
...
void print_battery_charger_info()
{
	...
	gfx_printf("Eingangsspannungsgrenze:       %4d mV\n", value);

	bq24193_get_property(BQ24193_InputCurrentLimit, &value);
	gfx_printf("Eingangsstrombegrenzung:       %4d mA\n", value);

	bq24193_get_property(BQ24193_SystemMinimumVoltage, &value);
	gfx_printf("Min. Spannungsgrenze:         %4d mV\n", value);

	bq24193_get_property(BQ24193_FastChargeCurrentLimit, &value);
	gfx_printf("Schnellladestrombegrenzung: %4d mA\n", value);

	bq24193_get_property(BQ24193_ChargeVoltageLimit, &value);
	gfx_printf("Ladespannungsgrenze:      %4d mV\n", value);

	bq24193_get_property(BQ24193_ChargeStatus, &value);
	gfx_printf("Ladestatus:             ");
	switch (value)
	{
	case 0:
		gfx_printf("Nicht geladen\n");
		break;
	case 1:
		gfx_printf("Vorm Laden\n");
		break;
	case 2:
		gfx_printf("Schnelles Aufladen\n");
		break;
	case 3:
		gfx_printf("Laden abgebrochen\n");
		break;
	default:
		gfx_printf("Unbekannt (%d)\n", value);
		break;
	}
	bq24193_get_property(BQ24193_TempStatus, &value);
	gfx_printf("Temperaturstatus:        ");
	switch (value)
	{
	case 0:
		gfx_printf("Normal\n");

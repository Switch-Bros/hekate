...
static u8 _hid_transfer_start(usb_ctxt_t *usbs, u32 len)
{
	...
	{
		usbs->set_text(usbs->label, "#FFDD00 Fehler:# EP-Eingangsuebertragung!");
...
int usb_device_gadget_hid(usb_ctxt_t *usbs)
{
	...
	usbs->set_text(usbs->label, "#C7EA46 Status:# Warten auf Verbindung");

	// Initialize Control Endpoint.
	if (usb_ops.usb_device_enumerate(gadget_type))
		goto error;

	usbs->set_text(usbs->label, "#C7EA46 Status:# Warten auf HID-Berichtsanfrage");

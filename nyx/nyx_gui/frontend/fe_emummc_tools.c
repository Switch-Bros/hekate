...
static int _dump_emummc_file_part(emmc_tool_gui_t *gui, char *sd_path, sdmmc_storage_t *storage, const emmc_part_t *part)
{
	...
	{
		s_printf(gui->txt_buf, "\n#FFDD00 Nicht genug freier Speicher fuer Dateibasiertes emuMMC!#\n");
	...
	if (res)
	{
		s_printf(gui->txt_buf, "\n#FF0000 Fehler (%d) beim erstellen von#\n#FFDD00 %s#\n", res, outFilename);
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
	...
	while (totalSectors > 0)
	{
		...
		if (res)
		{
			s_printf(gui->txt_buf, "\n#FF0000 Schwerwiegender Fehler (%d) beim schreiben auf die SD-Karte#\nBitte erneut versuchen...\n", res);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
...
void dump_emummc_file(emmc_tool_gui_t *gui)
{
	...
	lv_label_set_text(gui->label_info, "Vorhandener freier Speicher wird ueberprueft...");
	manual_system_maintenance(true);
...
static int _emummc_raw_derive_bis_keys(emmc_tool_gui_t *gui, u32 resized_count)
{
	...
	if (error)
	{
		...
		lv_mbox_set_text(mbox, "#C7EA46 Generiere BIS-Keys#");
...
void dump_emummc_raw(emmc_tool_gui_t *gui, int part_idx, u32 sector_start, u32 resized_count)
{
	...
	{
		s_printf(gui->txt_buf, "#FFDD00 Um USER Partition zu formatieren,#\n#FFDD00 werden BIS-Keys benoetigt!#\n");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		emmc_end();
		goto out;
	}

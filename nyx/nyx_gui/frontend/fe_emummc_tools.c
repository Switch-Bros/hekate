...
static int _dump_emummc_file_part(emmc_tool_gui_t *gui, char *sd_path, sdmmc_storage_t *storage, emmc_part_t *part)
{
	...
	s_printf(gui->txt_buf, "#96FF00 SD-Karte freier Speicher:# %d MB\n#96FF00 Gesamtgroesse:# %d MB\n\n",
	...
	{
		s_printf(gui->txt_buf, "\n#FFDD00 Nicht genug Platz fuer Dateibasiertes emuMMC!#\n");
	...
	while (totalSectors > 0)
	{
		...
		{
			s_printf(gui->txt_buf, "\n#FFDD00 emuMMC wurde abgebrochen!#\n");
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
		...
		while (!sdmmc_storage_read(storage, lba_curr, num, buf))
		{
			...
			if (retryCount >= 3)
			{
				s_printf(gui->txt_buf, "#FF0000 Wird abgebrochen...#\nBitte nochmal versuchen...\n");
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
...
void dump_emummc_file(emmc_tool_gui_t *gui)
{
	...
	if (!sd_mount())
	{
		lv_label_set_text(gui->label_info, "#FFDD00 SD-Karte konnt nicht initialisiert werden!#");
		goto out;
	}

	lv_label_set_text(gui->label_info, "Ueberpruefung des freien Speicherplatzes...");
	manual_system_maintenance(true);
	...
	{
		lv_label_set_text(gui->label_info, "#FFDD00 eMMC konnt nicht initialisiert werden!#");
		goto out;
	}
...
static int _dump_emummc_raw_part(emmc_tool_gui_t *gui, int active_part, int part_idx, u32 sd_part_off, emmc_part_t *part, u32 resized_count)
{
	...
	if (resized_count)
	{
		...
		if (!user_part)
		{
			s_printf(gui->txt_buf, "\n#FFDD00 USER Partition nicht gefunden!#\n");
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
	...
	if (resized_count)
	{
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, "Abgeschlossen!\n");
		...
		s_printf(gui->txt_buf, "Formatiere USER... \n");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
		...
		if (mkfs_error)
		{
			s_printf(gui->txt_buf, "#FF0000 Fehlgeschlagen (%d)!#\nBitte nochmal versuchen...\n", mkfs_error);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		...
		s_printf(gui->txt_buf, "Schreibe neues GPT... ");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
...
static int _emummc_raw_derive_bis_keys(emmc_tool_gui_t *gui, u32 resized_count)
{
	...
	if (memcmp(&cal0->magic, "CAL0", 4))
	{
		...
		strcpy(txt_buf, "#FFDD00 BIS keys Validierung fehlgeschlagen!#\n");
		error = true;
	}
	...
	if (error)
	{
		...
		lv_mbox_set_text(mbox, "#C7EA46 BIS Keys Erstellung#");
...
void dump_emummc_raw(emmc_tool_gui_t *gui, int part_idx, u32 sector_start, u32 resized_count)
{
	...
	{
		s_printf(gui->txt_buf, "#FFDD00 Zum formatieren der USER Partition,#\n#FFDD00 werden BIS keys benoetigt!#\n");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		emmc_end();
		goto out;
	}

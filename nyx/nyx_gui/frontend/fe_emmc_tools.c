	...
	if (f_open(&fp, outFilename, FA_READ) == FR_OK)
	{
		...
		while (totalSectorsVer > 0)
		{
			...
			if ((n_cfg.verification >= 2) || !(sparseShouldVerify % 4))
			{
				...
				{
					s_printf(gui->txt_buf,
						"\n#FF0000 SD-Karte & eMMC Daten (@LBA %08X) stimmen nicht ueberein!#\n"
			...
			{
				s_printf(gui->txt_buf, "#FFDD00 Verifizierung wurde abgebrochen!#\n");
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
...
static int _dump_emmc_part(emmc_tool_gui_t *gui, char *sd_path, int active_part, sdmmc_storage_t *storage, emmc_part_t *part)
{
	...
	if (gui->raw_emummc)
	{
		...
		{
			s_printf(gui->txt_buf, "\n#FFDD00 Partition nicht gefunden...#\n");
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
	...
	while (totalSectors > 0)
	{
		if (numSplitParts != 0 && bytesWritten >= multipartSplitSize)
		{
			...
			if (n_cfg.verification && !gui->raw_emummc)
			{
				...
				{
					s_printf(gui->txt_buf, "#FFDD00 Bitte nochmal versuchen...#\n");
					lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
					manual_system_maintenance(true);
			...
			if (isSmallSdCard)
			{
				...
				{
					s_printf(gui->txt_buf, "#FF0000 Fehler beim erstellen der partial.idx Datei!#\n");
					lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
					manual_system_maintenance(true);
				...
				if (currPartIdx >= maxSplitParts)
				{
					create_mbox_text(
						...
						"#96FF00 3.# SD-Karte wieder einlegen.\n"
						"#96FF00 4.# Waehle erneut die GLEICHE Option, um fortzufahren..", true);
		...
		while (res_read)
		{
			...
			{
				s_printf(gui->txt_buf, "#FFDD00 Erneuter Versuch...#\n");
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
			}
		}
		...
		if (res)
		{
			s_printf(gui->txt_buf, "\n#FF0000 Fataler Fehler (%d) beim schreiben auf SD-Karte#\nBitte nochmal versuchen...\n", res);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
	...
	if (n_cfg.verification && !gui->raw_emummc)
	{
		...
		if (_dump_emmc_verify(gui, storage, lbaStartPart, outFilename, part))
		{
			s_printf(gui->txt_buf, "\n#FFDD00 Bitte nochmal versuchen...#\n");
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
	...
	if (isSmallSdCard)
	{
		...
		create_mbox_text(
			...
			"Du kannst die Dateien bei Bedarf zusammenfuehren\num eine komplette eMMC RAW GPP Backupdatei zu erhalten.", true);
...
void dump_emmc_selected(emmcPartType_t dumpType, emmc_tool_gui_t *gui)
{
	...
	lv_label_set_text(gui->label_info, "Ueberpruefe freien Speicherplatz...");
	manual_system_maintenance(true);
...
static int _restore_emmc_part(emmc_tool_gui_t *gui, char *sd_path, int active_part, sdmmc_storage_t *storage, emmc_part_t *part, bool allow_multi_part)
{
	...
	if (f_stat(outFilename, &fno))
	{
		...
		s_printf(gui->txt_buf, "\nKeine einzelne Datei, pruefe auf Teil-Dateien...\n");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
		...
		while ((u32)((u64)totalCheckFileSize >> (u64)9) != totalSectors)
		{
			...
			{
				s_printf(gui->txt_buf, "#FF8000 Groesse der Teil-Dateien auf SD-Karte ist#\n#FF8000 groesser als die gewaehlte vom eMMC!#\n#FFDD00 Abbruch...#");
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
			...
			else if (f_stat(outFilename, &fno))
			{
				if (!gui->raw_emummc)
				{
					s_printf(gui->txt_buf, "#FFDD00 Fehler (%d) Datei nicht gefunden#\n#FFDD00 %s.#\n\n", res, outFilename);
					lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
					manual_system_maintenance(true);
				...
				if (!numSplitParts)
				{
					s_printf(gui->txt_buf, "#FFDD00 Der Restore Ordner ist leer.#\n\n");
					lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
					manual_system_maintenance(true);
			...
			else
			{
				...
				{
					s_printf(gui->txt_buf, "#FFDD00 Die Teil-Datei muss ein#\n#FFDD00 vielfaches von 4 MB sein.#\n#FFDD00 Abbruch...#");
					lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
					manual_system_maintenance(true);
		...
		if ((u32)((u64)totalCheckFileSize >> (u64)9) != totalSectors)
		{
			...
			{
				lv_obj_del(warn_mbox_bg);
				s_printf(gui->txt_buf, "#FF0000 Groesse der Teil-Dateien auf SD-Karte ist#\n#FF0000 groesser als die gewaehlte vom eMMC!#\n");
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
...
multipart_not_allowed:
	...
	if (res)
	{
		if (res != FR_NO_FILE)
		{
			s_printf(gui->txt_buf, "\n#FF0000 Fehler (%d) beim oeffnen der Datei. Fahre fort...#\n", res);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
		}
		else
		{
			s_printf(gui->txt_buf, "\n#FFDD00 Fehler (%d) Datei nicht gefunden. Fahre fort...#\n", res);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
		}
	...
	else if (!use_multipart && (((u32)((u64)f_size(&fp) >> (u64)9)) != totalSectors)) // Check total restore size vs emmc size.
	{
		...
		{
			s_printf(gui->txt_buf, "#FF8000 Groesse des Backups auf SD-Karte, ist#\n#FF8000 groesser als die gewaehlte vom eMMC!#\n#FFDD00 Abbruch...#");
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
		...
		{
			lv_obj_t *warn_mbox_bg = create_mbox_text(
				"#FF8000 Groesse des Backups auf SD-Karte, ist#\n#FF8000 groesser als die gewaehlte vom eMMC!#\n\n"
			...
			{
				lv_obj_del(warn_mbox_bg);
				s_printf(gui->txt_buf, "\n#FF0000 Groesse des Backups auf SD-Karte, ist#\n#FF0000 groesser als die gewaehlte vom eMMC.#\n");
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
	...
	while (totalSectors > 0)
	{
		...
		{
			s_printf(gui->txt_buf,
				"\n#FF0000 Fataler Fehler (%d) beim lesen von SD-Karte!#\n"
				"#FF0000 Dieses Geraet ist moeglicherweise nicht funktionsfaehig!#\n"
				"#FFDD00 Bitte jetzt nochmal versuchen!#\n", res);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
		...
		{
			s_printf(gui->txt_buf,
				"\n#FFDD00 Schreibfehler bei %d Bloecke @ LBA %08X,#\n"
				"#FFDD00 ins eMMC (Versuch %d). #",
				num, lba_curr, ++retryCount);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
...
void restore_emmc_selected(emmcPartType_t restoreType, emmc_tool_gui_t *gui)
{
	...
	s_printf(txt_buf,
		"#FFDD00 Das Geraet kann funktionsunfaehig werden!#\n\n"
		"#FFDD00 Bist du wirklich sicher?#");
	if ((restoreType & PART_BOOT) || (restoreType & PART_GP_ALL))
	{
		s_printf(txt_buf + strlen(txt_buf),
			"\n\nDer Modus den du gewaehlt hast stellt nur\ndie Partitionen wieder her die gefunden werden.\n"
			"Wenn es nicht gefunden wird, wird es uebersprungen\nund zum naechsten Schritt uebergegangen.");
	}
	...
	if (!sd_mount())
	{
		lv_label_set_text(gui->label_info, "#FFDD00 SD-Karte kann nicht initialisiert werden!#");
		goto out;
	}

	if (!emmc_initialize(false))
	{
		lv_label_set_text(gui->label_info, "#FFDD00 eMMC kann nicht initialisiert werden!#");
		goto out;
	}

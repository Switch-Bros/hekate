...
static void _check_sd_card_removed(void *params)
{
	...
	if (!do_reload && sd_get_card_removed())
	{
		...
		static const char * mbox_btn_map[] = { "\221Neustart (RCM)", "\221Ausschalten", "\221Nicht neu laden", "" };
		static const char * mbox_btn_map_rcm_patched[] = { "\221Neustart", "\221Ausschalten", "\221Nicht neu laden", "" };
		lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
		lv_mbox_set_recolor_text(mbox, true);
		lv_obj_set_width(mbox, LV_HOR_RES * 6 / 9);

		lv_mbox_set_text(mbox, "\n#FF8000 SD-Karte wurde entfernt!#\n\n#96FF00 Nyx wird neu geladen wenn sie eingesteckt wird.#\n\nDenk daran dass du UMS nutzen kannst anstatt sie zu entfernen.\n");
...
static void _nyx_emmc_issues(void *params)
{
	if (emmc_get_mode() < EMMC_MMC_HS400)
	{
		...
		lv_mbox_set_text(mbox,
			"#FF8000 eMMC Fehlerueberpruefung#\n\n"
			"#FFDD00 Dein eMMC wird im langsamen Modus initialisiert!#\n"
			"#FFDD00 Dies koennte ein Hardware-Problem sein!#\n\n"
			"Schau mal in\n#C7EA46 Konsoleninfo# -> #C7EA46 eMMC#");
...
static lv_res_t _create_mbox_reload(lv_obj_t *btn)
{
	...
	static const char * mbox_btn_map[] = { "\221Neu laden", "\221Abbrechen", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES * 4 / 10);

	lv_mbox_set_text(mbox, "#FF8000 Moechtest du wirklich#\n#FF8000 hekate & Nyx neu laden?#\n\n"
		"Dies ueberprueft auch\n#96FF00 bootloader/update.bin#\nfuer hekate Updates");
...
static lv_res_t _create_mbox_reboot(lv_obj_t *btn)
{
	...
	static const char * mbox_btn_map[] = { "\221OFW", "\221RCM", "\221Abbrechen", "" };
	static const char * mbox_btn_map_autorcm[] = { "\261OFW", "\221RCM", "\221Abbrechen", "" };
	static const char * mbox_btn_map_patched[] = { "\221OFW", "\221Normal", "\221Abbrechen", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 2);

	lv_mbox_set_text(mbox, "#FF8000 Waehle wohin neugestartet werden soll:#");
...
static lv_res_t _create_mbox_poweroff(lv_obj_t *btn)
{
	...
	static const char * mbox_btn_map[] = { "\221Ausschalten", "\221Abbrechen", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES * 4 / 10);
...
void nyx_create_onoff_button(lv_theme_t *th, lv_obj_t *parent, lv_obj_t *btn, const char *btn_name, lv_action_t action, bool transparent)
{
	...
	lv_label_set_text(label_btnsw, "#D0D0D0 AUS#");
...
static void _create_tab_about(lv_theme_t * th, lv_obj_t * parent)
{
	...
	lv_label_set_static_text(lbl_credits,
		...
		"\n"
		"Dank an: #00CCFF derrek, nedwill, plutoo, #\n"
		"           #00CCFF shuffle2, smea, thexyz, yellows8 #\n"
		"\n"
		"Gruesse an: fincs, hexkyz, SciresM,\n"
		"              Shiny Quagsire, WinterMute\n"
		"\n"
		"Genutzte Open Source und kostenlose Pakete:\n\n"
		" - FatFs R0.13c,\n"
	...
	char version[32];
	s_printf(version, "Nyx %d.%d.%d SB", NYX_VER_MJ, NYX_VER_MN, NYX_VER_HF);
...
static lv_res_t _create_mbox_payloads(lv_obj_t *btn)
{
	...
	static const char * mbox_btn_map[] = { "\251", "\222Abbrechen", "\251", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES * 5 / 9);

	lv_mbox_set_text(mbox, "Waehle payload zum starten aus:");
	...
	if (!sd_mount())
	{
		lv_mbox_set_text(mbox, "#FFDD00 SD-Karte konnte nicht initialisiert werden!#");
...
static lv_res_t _create_window_home_launch(lv_obj_t *btn)
{
	...
	if (btn)
	{
		if (strcmp(lv_label_get_text(lv_obj_get_child(btn, NULL)) + 8,"Starten#"))
			more_cfg = true;
	}
	else
	{
	...
	if (!btn)
		win = create_window_launch(SYMBOL_GPS" hekate - Starten");
	else if (!more_cfg)
		win = create_window_launch(SYMBOL_GPS" Starten");
	else
		win = create_window_launch(SYMBOL_GPS" Weitere Konfigs.");

	lv_win_add_btn(win, NULL, SYMBOL_LIST" Logs #D0D0D0 AUS#", logs_onoff_toggle);
	...
	if (no_boot_entries)
	{
		...
		if (!more_cfg)
		{
			lv_label_set_static_text(label_error,
				"#FFDD00 Keine Haupteintraege gefunden...#\n"
				"Ueberpruefe ob#96FF00 bootloader/hekate_ipl.ini# Starteintraege hat\n"
				"oder nutze den#C7EA46 Weitere Konfigs# Button fuer weitere Eintraege.");
		}
		else
		{
			lv_label_set_static_text(label_error,
				"#FFDD00 Keine .ini oder Starteintraege gefunden...#\n"
				"Ueberpruefe ob eine .ini Datei im #96FF00 bootloader/ini/ Verzeichnis ist#\n"
				"und mindestens einen Eintrag enthaelt.");
		}
...
static void _create_tab_home(lv_theme_t *th, lv_obj_t *parent)
{
	...
	lv_label_set_static_text(label_tagline, "DER ALLUMFASSENDE BOOTLOADER FUER ALLE DEINE BEDUERFNISSE");
	...
	s_printf(btn_colored_text, "%s%s", text_color, " Starten#");
	...
	_create_text_button(th, NULL, btn_nyx_options, SYMBOL_SETTINGS" Nyx Einstellungen", NULL);
	...
	_create_text_button(th, NULL, btn_power_off, SYMBOL_POWER" Ausschalten", _create_mbox_poweroff);
	lv_obj_align(btn_power_off, NULL, LV_ALIGN_IN_BOTTOM_RIGHT, -LV_DPI / 4, -LV_DPI / 12);

	_create_text_button(th, NULL, btn_reboot, SYMBOL_REBOOT" Neustart", _create_mbox_reboot);
	lv_obj_align(btn_reboot, btn_power_off, LV_ALIGN_OUT_LEFT_MID, 0, 0);

	_create_text_button(th, NULL, btn_reload, SYMBOL_REFRESH" Neu laden", _create_mbox_reload);
...
static lv_res_t _save_options_action(lv_obj_t *btn)
{
	...
	if (res)
		lv_mbox_set_text(mbox, "#FF8000 hekate Konfiguration#\n\n#96FF00 Die Konfiguration wurde auf SD-Karte gespeichert!#");
	else
		lv_mbox_set_text(mbox, "#FF8000 hekate Konfiguration#\n\n#FFDD00 Fehler beim speichern der Konfiguration#\n#FFDD00 auf SD-Karte!#");
...
void nyx_check_ini_changes()
{
	if (nyx_options_get_ini_changes_made())
	{
		...
		static const char * mbox_btn_map[] = { "\222Speichern", "\222Abbrechen", "" };
		lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
		lv_mbox_set_recolor_text(mbox, true);

		lv_mbox_set_text(mbox,
			"#FF8000 Haupt-Konfiguration#\n\n"
			"Die Konfiguration wurde geaendert!\n\n"
			"Moechtest du sie speichern?");
...
static void _nyx_main_menu(lv_theme_t * th)
{
	...
	char version[32];
	s_printf(version, "hekate %d.%d.%d SB", nyx_str->version & 0xFF, (nyx_str->version >> 8) & 0xFF, (nyx_str->version >> 16) & 0xFF);
	...
	lv_obj_t *tab_info = lv_tabview_add_tab(tv, SYMBOL_INFO" Konsoleninfo");
	lv_page_set_style(tab_info, LV_PAGE_STYLE_BG, &no_padding);
	lv_page_set_style(tab_info, LV_PAGE_STYLE_SCRL, &no_padding);

	lv_obj_t *tab_options = lv_tabview_add_tab(tv, SYMBOL_SETTINGS" Optionen");

...
static int _stat_and_copy_files(const char *src, const char *dst, char *path, u32 *total_files, u32 *total_size, lv_obj_t **labels)
{
	...
	for (;;)
	{
		...
		{
			if (!memcmp("Systemdatentraeger-Informationen", fno.fname, 25))
				continue;
...
static void _prepare_and_flash_mbr_gpt()
{
	...
	if (part_info.and_size)
	{
		...
		// Set GPT header.
		memcpy(&gpt->header.signature, "EFI PARTITION", 8);
...
static lv_res_t _action_flash_linux_data(lv_obj_t * btns, const char * txt)
{
	...
	static const char *mbox_btn_map2[] = { "\223Loesche Installationsdateien", "\221OK", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 10 * 5);

	lv_mbox_set_text(mbox, "#FF8000 Linux Erstellen#");

	lv_obj_t *lbl_status = lv_label_create(mbox, NULL);
	lv_label_set_recolor(lbl_status, true);
	lv_label_set_text(lbl_status, "#C7EA46 Status:# Erstelle Linux...");
	...
	if (res)
	{
		lv_label_set_text(lbl_status, "#FFDD00 Fehler:# Erste Teil konnte nicht geoeffnet werden!");
	...
	while (total_size_sct > 0)
	{
		...
		if (bytesWritten >= fileSize)
		{
			...
			if (res)
			{
				s_printf(txt_buf, "#FFDD00 Fehler:# Teil %d konnte nicht geoeffnet werden#", currPartIdx);
		...
		if (res)
		{
			lv_label_set_text(lbl_status, "#FFDD00 Fehler:# Beim Lesen von SD-Karte!");
			manual_system_maintenance(true);
		...
		while (res)
		{
			...
			if (retryCount >= 3)
			{
				lv_label_set_text(lbl_status, "#FFDD00 Fehler:# Beim Schreiben auf SD-Karte!");
				manual_system_maintenance(true);
...
static u32 _get_available_l4t_partition()
{
	...
	u32 size_sct = 0;
	if (!memcmp(&gpt->header.signature, "EFI PARTITION", 8) || gpt->header.num_part_ents > 128)
	{
...
static lv_res_t _action_check_flash_linux(lv_obj_t *btn)
{
	...
	static const char *mbox_btn_map[] = { "\251", "\222OK", "\251", "" };
	static const char *mbox_btn_map2[] = { "\222Weiter", "\222Abbrechen", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	...
	lv_obj_t *lbl_status = lv_label_create(mbox, NULL);
	lv_label_set_recolor(lbl_status, true);
	lv_label_set_text(lbl_status, "#C7EA46 Status:# Suche nach Dateien und Partitionen...");
	...
	if (f_stat(path, NULL))
	{
		lv_label_set_text(lbl_status, "#FFDD00 Fehler:# Installationsdaten nicht gefunden!");
		goto error;
	}
	...
	{
		lv_label_set_text(lbl_status, "#FFDD00 Fehler:# Keine Partition gefunden!");
		goto error;
	}
	...
	while (true)
	{
		...
		if ((u64)fno.fsize % SZ_4M)
		{
			...
			{
				lv_label_set_text(lbl_status, "#FFDD00 Fehler:# Das Image ist nicht auf 4 MiB ausgerichtet!");
				goto error;
			}
	...
	{
		lv_label_set_text(lbl_status, "#FFDD00 Fehler:# Das Image ist groesser als die Partition!");
		goto error;
	}

	char *txt_buf = malloc(SZ_4K);
	s_printf(txt_buf,
		"#C7EA46 Status:# Installationsdateien und Partition gefunden.\n"
		"#00DDFF Beginn:# %08x, #00DDFF Groesse:# %X, #00DDFF Imagegroesse:# %d MiB\n"
...
static lv_res_t _action_flash_android_data(lv_obj_t * btns, const char * txt)
{
	...
	static const char *mbox_btn_map[] = { "\251", "\222OK", "\251", "" };
	static const char *mbox_btn_map2[] = { "\222Weiter", "\222Nein", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);

	lv_mbox_set_text(mbox, "#FF8000 Android Erstellen#");

	lv_obj_t *lbl_status = lv_label_create(mbox, NULL);
	lv_label_set_recolor(lbl_status, true);
	lv_label_set_text(lbl_status, "#C7EA46 Status:# Suche nach Deteien und Partitionen...");
	...
	if (memcmp(&gpt->header.signature, "EFI PARTITION", 8) || gpt->header.num_part_ents > 128)
	{
		lv_label_set_text(lbl_status, "#FFDD00 Fehler:# Android GPT wurde nicht gefunden!");
		goto error;
	}
	...
	if (f_stat(path, NULL))
	{
		s_printf(txt_buf, "#FF8000 Warnung:# Kernel-Image nicht gefunden!\n");
	...
	if (offset_sct && size_sct)
	{
		...
		if ((file_size >> 9) > size_sct)
			s_printf(txt_buf, "#FF8000 Warnung:# Kernel-Image zu gross!\n");
		else
		{
			sdmmc_storage_write(&sd_storage, offset_sct, file_size >> 9, buf);

			s_printf(txt_buf, "#C7EA46 Erfolg:# Kernel-Image geschrieben!\n");
			f_unlink(path);
		}
		...
		s_printf(txt_buf, "#FF8000 Warnung:# Kernel Partition nicht gefunden!\n");
...
boot_img_not_found:
	...
	if (f_stat(path, NULL))
	{
		...
		if (f_stat(path, NULL))
		{
			strcat(txt_buf, "#FF8000 Warnung:# Recovery-Image nicht gefunden!\n");
	...
	if (offset_sct && size_sct)
	{
		...
		if ((file_size >> 9) > size_sct)
			strcat(txt_buf, "#FF8000 Warnung:# Recovery-Image zu gross!\n");
		else
		{
			sdmmc_storage_write(&sd_storage, offset_sct, file_size >> 9, buf);
			strcat(txt_buf, "#C7EA46 Erfolg:# Recovery-Image geschrieben!\n");
			f_unlink(path);
		}
		...
		strcat(txt_buf, "#FF8000 Warnung:# Recovery Partition nicht gefunden!\n");
...
recovery_not_found:
	...
	if (f_stat(path, NULL))
	{
		...
		if (f_stat(path, NULL))
		{
			strcat(txt_buf, "#FF8000 Warnung:# DTB-Image nicht gefunden!");
			goto dtb_not_found;
		}
	}
	...
	if (offset_sct && size_sct)
	{
		...
		if ((file_size >> 9) > size_sct)
			strcat(txt_buf, "#FF8000 Warnung:# DTB-Image zu gross!");
		else
		{
			sdmmc_storage_write(&sd_storage, offset_sct, file_size >> 9, buf);
			strcat(txt_buf, "#C7EA46 Erfolg:# DTB-Image geschrieben!");
			f_unlink(path);
		}
		...
		strcat(txt_buf, "#FF8000 Warnung:# DTB Partition nicht gefunden!");
...
error:
	if (boot_recovery)
	{
		...
		strcat(txt_buf,"\n\nMoechtest du in die Recovery starten\num die Android-Installation abzuschliessen?");
		lv_label_set_text(lbl_status, txt_buf);
...
static lv_res_t _action_flash_android(lv_obj_t *btn)
{
	...
	static const char *mbox_btn_map[] = { "\222Weiter", "\222Abbrechen", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);
	...
	lv_label_set_text(lbl_status,
		"Dies erstellt, wenn gefunden, den #C7EA46 Kernel#, #C7EA46 DTB# und #C7EA46 Recovery.#\n"
		"Nach erfolgreichem erstellen werden sie geloescht.\n"
...
static lv_res_t _create_mbox_start_partitioning(lv_obj_t *btn)
{
	...
	static const char *mbox_btn_map[] =  { "\251", "\222OK", "\251", "" };
	static const char *mbox_btn_map1[] = { "\222SD UMS-Modus", "\222Linux Erstellen", "\222Android Erstellen", "\221OK", "" };
	static const char *mbox_btn_map2[] = { "\222SD UMS-Modus", "\222Linux Erstellen", "\221OK", "" };
	static const char *mbox_btn_map3[] = { "\222SD UMS-Modus", "\222Android Erstellen", "\221OK", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);
	...
	while (seconds)
	{
		s_printf(txt_buf + text_idx, "%d Sekunden...", seconds);
		lv_mbox_set_text(mbox, txt_buf);
		manual_system_maintenance(true);
		msleep(1000);
		seconds--;
	...
	lv_mbox_set_text(mbox,
		...
		"#FFDD00 Warnung: Willst du wirklich fortfahren?!#\n\n"
	...
	lv_label_set_text(lbl_status, "#00DDFF Status:# Initialisiere Speicherlaufwerk...");
	...
	{
		lv_label_set_text(lbl_status, "#FFDD00 Fehler:# Initialisieren des Speicherlaufwerks fehlgeschlagen!");
		goto error;
	}

	lv_label_set_text(lbl_status, "#00DDFF Status:# Dateien werden gesichert...");
	manual_system_maintenance(true);
	...
	{
		if (part_info.backup_possible)
			lv_label_set_text(lbl_status, "#FFDD00 Fehler:# Sichern der Dateien fehlgeschlagen!");
		else
			lv_label_set_text(lbl_status, "#FFDD00 Fehler:# Dateien konnten nicht gesichert werden!\nBootloader Ordner ist groesser 1GB oder beschaedigt!");
	...
	{
		// Failed to format.
		s_printf((char *)buf, "#FFDD00 Fehler:# Formatieren des Datentraegers (%d) fehlgeschlagen!\n\n"
			"Entferne die SD-Karte und ueberpruefe ob sie OK ist.\nWenn nicht, formatiere sie,\nsteck sie ein und Druecke #FF8000 POWER#!", mkfs_error);
		...
		if (_backup_and_restore_files(false, lbl_paths))
		{
			...
			if (_backup_and_restore_files(false, lbl_paths))
			{
				lv_label_set_text(lbl_status, "#FFDD00 Fehler:# Wiederherstellen der Dateien fehlgeschlagen!");
				free(buf);
				goto error;
			}
		}
...
mkfs_no_error:
	...
	if (_backup_and_restore_files(false, lbl_paths))
	{
		...
		if (_backup_and_restore_files(false, lbl_paths))
		{
			lv_label_set_text(lbl_status, "#FFDD00 Fehler:# Dateien wiederherstellen fehlgeschlagen!");
			goto error;
		}
	}
	...
	lv_label_set_text(lbl_status, "#00DDFF Status:# Erstelle Partitionstabelle...");
	lv_label_set_text(lbl_paths[0], "Bitte warten...");
	lv_label_set_text(lbl_paths[1], " ");
	manual_system_maintenance(true);
	...
	sd_unmount();
	lv_label_set_text(lbl_status, "#00DDFF Status:# Abgeschlossen!");
	manual_system_maintenance(true);
...
static lv_res_t _create_mbox_partitioning_warn(lv_obj_t *btn)
{
	...
	static const char *mbox_btn_map[] = { "\222SD UMS-Modus", "\222Start", "\222Abbrechen", "" };
	static const char *mbox_btn_map2[] = { "\222Start", "\222Abbrechen", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	...
	s_printf(txt_buf, "#FFDD00 Warnung: Dies formatiert die SD-Karte!#\n\n");

	if (part_info.backup_possible)
	{
		strcat(txt_buf, "#C7EA46 Dateien werden gesichert und wiederhergestellt!#\n"
			"#FFDD00 Jede andere Partition wird geloescht!#");
	}
	else
	{
		strcat(txt_buf, "#FFDD00 Deine Dateien werden geloescht!#\n"
			"#FFDD00 Jede andere Partition wird ebenfalls geloescht!#\n"
			"#FFDD00 Benutze USB UMS-Modus um sie umzukopieren!#");
	}
...
static lv_res_t _create_mbox_partitioning_andr_part(lv_obj_t *btn)
{
	...
	static const char *mbox_btn_map[] = { "\222Dynamisch", "\222Legacy", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);

	lv_obj_set_width(mbox, LV_HOR_RES / 10 * 5);
	lv_mbox_set_text(mbox, "#FF8000 Android Partitionierung#");

	lv_obj_t *lbl_status = lv_label_create(mbox, NULL);
	lv_label_set_recolor(lbl_status, true);

	lv_label_set_text(lbl_status,
		"Waehle ein Partitionsschema:\n\n"
		"#C7EA46 Dynamisch:# Android 13+\n"
...
static void _create_mbox_check_files_total_size()
{
	...
	static const char *mbox_btn_map[] = { "\251", "\222OK", "\251", "" };
	static const char *mbox_btn_map2[] = { "\222Keine Sicherung", "\222OK", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);

	lv_mbox_set_text(mbox, "Analysiere SD-Kartennutzung. Das kann eine weile dauern...");
	...
	{
		s_printf(txt_buf,
			"#96FF00 SD-Kartendaten werden automatisch gesichert!#\n"
			"#FFDD00 Jede andere Partition wird geloescht!#\n"
			"#00DDFF Dateien insgesamt:# %d, #00DDFF Dateigroesse:# %d MiB", total_files, total_size >> 20);
		lv_mbox_set_text(mbox, txt_buf);
	}
	else
	{
		lv_mbox_set_text(mbox,
			"#FFDD00 SD-Karte kann NICHT automatisch gesichert werden!#\n"
			"#FFDD00 Jede andere Partition wird ebenfalls geloescht!#\n\n"
			"Du wirst spaeter gefragt ob du deine Dateien ueber\nueber den UMS-Modus sichern willst.");
	}
	...
	lv_label_set_text(lbl_part, "#00DDFF Aktuelles MBR Partitionslayout:#");
	...
	s_printf(txt_buf,
		"Partition 0 - Typ: %02x, Beginn: %08x, Groesse: %08x\n"
		"Partition 1 - Typ: %02x, Beginn: %08x, Groesse: %08x\n"
		"Partition 2 - Typ: %02x, Beginn: %08x, Groesse: %08x\n"
		"Partition 3 - Typ: %02x, Beginn: %08x, Groesse: %08x",
...
static lv_res_t _action_fix_mbr(lv_obj_t *btn)
{
	...
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);
	lv_mbox_set_text(mbox, "#FF8000 Hybrid MBR korrigieren#");
	...
	{
		lv_label_set_text(lbl_status, "#FFDD00 SD-Karte konnte nicht initialisiert werden!#");
		goto out;
	}
	...
	if (!gpt_partition_exists || memcmp(&gpt->header.signature, "EFI PARTITION", 8) || gpt->header.num_part_ents > 128)
	{
		lv_label_set_text(lbl_status, "#FFDD00 Warnung:# Keine gueltige GPT gefunden!");
...
check_changes:
	...
	{
		lv_label_set_text(lbl_status, "#96FF00 Warnung:# Hybrid MBR benoetigt keine Aenderung!#");
		goto out;
	}
	...
	// Current MBR info.
	s_printf(txt_buf, "#00DDFF Aktuelles MBR Layout:#\n");
	...
	// New MBR info.
	s_printf(txt_buf + strlen(txt_buf), "#00DDFF Neues MBR Layout:#\n");
	s_printf(txt_buf + strlen(txt_buf),
		"Partition 0 - Typ: %02x, Beginn: %08x, Groesse: %08x\n"
		"Partition 1 - Typ: %02x, Beginn: %08x, Groesse: %08x\n"
		"Partition 2 - Typ: %02x, Beginn: %08x, Groesse: %08x\n"
	...
	lv_label_set_text(lbl_status,
		"#FF8000 Warnung: Willst du wirklich fortfahren?!#\n\n"
		"Druecke #FF8000 POWER# um fortzufahren.\nDruecke #FF8000 VOL# um abzubrechen.");
	...
	if (btn_wait() & BTN_POWER)
	{
		...
		lv_label_set_text(lbl_status, "#96FF00 Neues Hybrid MBR erfolgreich erstellt!#");
	}
	else
		lv_label_set_text(lbl_status, "#FFDD00 Warnung: Hybrid MBR Korrektur abgebrochen!#");
...
lv_res_t create_window_partition_manager(lv_obj_t *btn)
{
	...
	lv_win_add_btn(win, NULL, SYMBOL_MODULES_ALT" Hybrid MBR korrigieren", _action_fix_mbr);
	...
	if (!sd_mount())
	{
		...
		lv_label_set_text(lbl, "#FFDD00 SD-Karte konnte nicht initialisiert werden!#");
		return LV_RES_OK;
	}
	...
	lv_obj_t *lbl = lv_label_create(h1, NULL);
	lv_label_set_recolor(lbl, true);
	lv_label_set_text(lbl, "Waehle #FFDD00 Neues# Partitionslayout:");
	...
	lv_label_set_static_text(lbl_notes,
		...
		"Hinweis 2: Groessenanpassung des emuMMC formatiert die USER-Partition. Speicherstaende sollten (extern) gesichert werden.\n"
		"Hinweis 3: #C7EA46 Erstelle Linux# und #C7EA46 Erstelle Android# schreibt Dateien, wenn Partitionen und Installationsdateien gefunden werden.\n");
	...
	lv_label_set_static_text(label_btn2, SYMBOL_DOWNLOAD"  Erstelle Linux");
	...
	case 0: // Disable Flash Android button if partition not found.
		lv_label_set_static_text(label_btn, SYMBOL_DOWNLOAD"  Erstelle Android");
		lv_obj_set_click(btn_flash_android, false);
		lv_btn_set_state(btn_flash_android, LV_BTN_STATE_INA);
		break;
	case 1: // Android 10/11.
		lv_label_set_static_text(label_btn, SYMBOL_DOWNLOAD"  Erstelle Android 10/11");
		break;
	case 2: // Android 13+
		lv_label_set_static_text(label_btn, SYMBOL_DOWNLOAD"  Erstelle Android 13+");
		break;
	}

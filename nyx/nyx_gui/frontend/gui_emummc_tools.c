...
static void _create_window_emummc()
{
	...
	lv_obj_t *win;
	if (!mbr_ctx.part_idx)
		win = nyx_create_window_custom_close_btn(SYMBOL_DRIVE"  Erstelle emuMMC-Datei auf SD-Karte", _action_emummc_window_close);
	else
		win = nyx_create_window_custom_close_btn(SYMBOL_DRIVE"  Erstelle emuMMC-Partition auf SD-Karte", _action_emummc_window_close);
...
static void _create_mbox_emummc_raw()
{
	...
	static const char *mbox_btn_format[] = { "\222Weiter", "\222Abbruch", "" };
	static char *mbox_btn_parts[] = { "\262Teil 1", "\262Teil 2", "\262Teil 3", "\222Abbruch", "" };
	...
	{
		s_printf(txt_buf,
			"#C7EA46 Anwendbare Partition(en) gefunden!#\n"
			"#FF8000 Waehle eine Partition, um fortzufahren:#\n\n");
	}
	else
		s_printf(txt_buf, "#FFDD00 Keine anwendbare Partition gefunden!#\n\n");
	...
	if (mbr_ctx.resized_cnt[0] || mbr_ctx.resized_cnt[1] || mbr_ctx.resized_cnt[2])
		strcat(txt_buf, "\n\n#FFDD00 Hinweis:# Gelbe Eintraege haben die USER-Partition vergroessert.");

	if (!mbr_ctx.available)
		strcat(txt_buf, "\n#FF8000 Moechtest du die SD-Karte partitionieren?#\n"
						  "#FF8000 (Du wirst gefragt, wie weiter verfahren werden soll.)#");
...
static lv_res_t _create_mbox_emummc_create(lv_obj_t *btn)
{
	...
	static const char * mbox_btn_map[] = { "\222SD Datei", "\222SD Partition", "\222Abbruch", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);

	lv_mbox_set_text(mbox,
		"Willkommen beim #C7EA46 emuMMC# Erstellungs-Tool!\n\n"
		"Bitte waehle den Typ vom emuMMC aus, den du erstellen moechtest.\n"
		"#FF8000 SD Datei# wird als Dateien in der FAT-Partition gespeichert.\n"
		"#FF8000 SD Partition# wird als RAW-Image in einer verfuegbaren Partition gespeichert.");
...
static void _create_emummc_migrated_mbox()
{
	...
	lv_mbox_set_text(mbox,
		...
		"#96FF00 Die emuMMC Konfiguration#\n#96FF00 wurde auf der SD-Karte gespeichert!#");
...
static lv_res_t _create_emummc_migrate_action(lv_obj_t * btns, const char * txt)
{
	...
	static const char *mbox_btn_map[] = { "\222Weiter", "\222Abbruch", "" };
	static const char *mbox_btn_map1[] = { "\222SD Datei", "\222SD Partition", "\222Abbruch", "" };
	...
	if (backup)
	{
		if (!emummc_backup)
			s_printf(txt_buf,
				"#C7EA46 Geeignete eMMC-Sicherung gefunden!#\n\n"
				"#FF8000 Moechtest du es migrieren?#\n");
		else
			s_printf(txt_buf,
				"#C7EA46 Geeignete emuMMC-Sicherung gefunden!#\n\n"
				"#FF8000 Moechtest du es migrieren?#\n");
	...
	{
		s_printf(txt_buf,
			"#C7EA46 SD-Partition basiertes emuMMC gefunden!#\n\n"
			"#FF8000 Moechtest du die Konfiguration und den Partitionstyp dafuer reparieren?#\n");
		lv_mbox_add_btns(mbox, mbox_btn_map, _create_emummc_mig3_action);
	}
	else if (em_raw && em_file)
	{
		s_printf(txt_buf,
			"#C7EA46 Sowohl fremde SD-Datei als auch Partition emuNAND gefunden!#\n\n"
			"#FF8000 Waehle aus, was migriert werden soll:#\n");
		lv_mbox_add_btns(mbox, mbox_btn_map1, _create_emummc_mig1_action);
	}
	else if (em_raw)
	{
		s_printf(txt_buf,
			"#C7EA46 Fremde SD-Partition emuNAND gefunden!#\n\n"
			"#FF8000 Moechtest du es migrieren?#\n");
	...
	{
		s_printf(txt_buf,
			"#C7EA46 Fremde SD-Datei emuNAND gefunden!#\n\n"
			"#FF8000 Moechtest du es migrieren?#\n");
...
static lv_res_t _create_mbox_emummc_migrate(lv_obj_t *btn)
{
	...
	static char *mbox_btn_map[] = { "\262Sicherung", "\262Rep. RAW", "\262EmuNAND", "\222Abbruch", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);
	...
	lv_mbox_set_text(mbox,
		...
		"Bitte waehle, welche Art der Migration du durchfuehren moechtest.\n"
		"Alles, was nicht gefunden wurde, hat den Button deaktiviert.");
...
out0:;
	...
	lv_label_set_static_text(label_txt3, "SD Dateibasiert");
...
lv_res_t create_win_emummc_tools(lv_obj_t *btn)
{
	...
	{
		lv_label_set_static_text(label_txt2, "emuMMC ist deaktiviert, eMMC wird zum Booten genutzt.\n\n");
	}
	...
	lv_label_set_static_text(label_txt2,
		"Wahle zwischen Images, die im emuMMC-Ordner oder in\n"
		"SD-Kartenpartitionen erstellt wurden. Es koennen maximal 3 partitionbasierte\n"
		"und unbegrenzt viele Dateibasierte Images vorhanden sein.");
	...
	lv_label_set_static_text(label_btn, SYMBOL_DRIVE"  Erstelle emuMMC");
	...
	lv_label_set_static_text(label_txt4,
		...
		"emuMMC zu erstellen. Du kannst es vom eMMC oder einem eMMC-Backup erstellen.");
	...
	lv_label_set_recolor(label_txt4, true);
	lv_label_set_static_text(label_txt4,
		"Migriere ein Backup zu einer #C7EA46 SD Datei# oder repariere eine bestehende #C7EA46 SD RAW Partition#.\n"

...
static lv_res_t _create_mbox_clock_edit(lv_obj_t *btn)
{
	...
	lv_mbox_set_text(mbox, "Gib #C7EA46 Datum# und #C7EA46 Zeit# fuer Nyx ein\nDies aendert nicht die tatsaechliche HW-Uhr!");
...
disabled:;
	...
	if (!error)
	{
		if (!nx_hoag)
		{
			...
			if (!success)
				strcat(txt_buf,
					"\n\n#FFDD00 Stelle sicher das beide JoyCons verbunden, und#\n"
					"#FFDD00 im HOS synchronisiert sind!#");
		}
		else
		{
			s_printf(txt_buf,
	...
	else
	{
		if (!nx_hoag)
			...
			s_printf(txt_buf, "#FFDD00 Warnung:# Lite Gamepad Info fehlt!#\n");
	}
...
lv_res_t create_win_nyx_options(lv_obj_t *parrent_btn)
{
	...
	lv_label_set_static_text(label_btn, SYMBOL_COPY" Farbeinstellung");
	...
	lv_label_set_static_text(label_txt2, "Waehle eine Farbe fuer alle #00FFC8 Highlights# in Nyx.\n");
	...
	lv_label_set_static_text(label_txt, SYMBOL_HOME" Startbildschirm");
	...
	nyx_create_onoff_button(th, sw_h2, btn2, SYMBOL_GPS" Erweiterte Starteintraege", _entries_columns_action, true);
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"Erlaubt das speichern der Switch und JoyCon MAC Adressen\n"
		"und den LTKs die damit verbunden sind. Fuer #C7EA46 Android# und #C7EA46 Linux#.");
	lv_obj_set_style(label_txt2, &hint_small_style);
...
void create_tab_options(lv_theme_t *th, lv_obj_t *parent)
{
	...
	lv_btn_set_fit(btn, true, true);
	lv_btn_set_toggle(btn, true);
	lv_label_set_static_text(label_btn, SYMBOL_GPS" Autostart #00FFC9   AN #");
